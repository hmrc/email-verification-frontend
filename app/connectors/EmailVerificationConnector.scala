...
class EmailVerificationConnector @Inject() (
  ...
  def submitEmail(journeyId: String, email: String)(implicit hc: HeaderCarrier): Future[SubmitEmailResponse] = {
    http.POST[JsValue, HttpResponse](
      s"$serviceUrl/email-verification/journey/$journeyId/email", Json.obj("email" -> email)
    ).map { response =>
  ...
  def validatePasscode(journeyId: String, passcode: String)(implicit hc: HeaderCarrier): Future[ValidatePasscodeResponse] = {
    http.POST[JsValue, HttpResponse](
      s"$serviceUrl/email-verification/journey/$journeyId/passcode", Json.obj("passcode" -> passcode)
    ).map { response =>
        Try(response.json) match {
          case Success(json) =>
...
object SubmitEmailResponse {
  ...
  implicit val reads: Reads[SubmitEmailResponse] = (json: JsValue) => (json \ "status").validate[String].flatMap {
...
object ValidatePasscodeResponse {
  ...
  implicit val reads: Reads[ValidatePasscodeResponse] = (json: JsValue) => (json \ "status").validate[String].flatMap {
...
object ResendPasscodeResponse {
  ...
  implicit val reads: Reads[ResendPasscodeResponse] = (json: JsValue) => (json \ "status").validate[String].flatMap {
